...
const ChatArea: React.FC<ChatAreaProps> = ({
  ...
  const handleScroll = () => {
    ...
    // 300px 이상 위로 올렸을 때만 플로팅 버튼이 보이도록 설정
  ...
  return (
    <section className="w-full bg-white flex flex-col h-full box-border font-sans relative">
      {/* 챗 상단 간결한 토글 트리거 영역 */}
      <div className="h-11 px-5 border-b border-solid border-[#e2e8f0] flex items-center justify-between bg-white shrink-0 box-border">
        <button
          onClick={onToggleSidebar}
          title={isSidebarOpen ? "최근 대화 닫기" : "최근 대화 열기"}
          className="group rounded-3xl p-1.5 px-2 cursor-pointer bg-white text-[#71717a] transition-all duration-150 flex items-center justify-center h-8 w-8 hover:bg-[#f4f4f5] hover:text-[#18181b]"
        >
          {/* 기본 사이드바 아이콘: 호버가 아닐 때 노출 */}
          <svg
            width="18"
            height="18"
            viewBox="0 0 24 24"
      ...
      <div
        ...
        className="custom-scrollbar flex-1 p-6 px-5 overflow-y-auto flex flex-col gap-6 bg-white"
      >
      ...
      {showScrollBottom && (
        <button
          ...
          className="absolute bottom-20 right-3 bg-white text-[#18181b] border border-solid border-[#cbd5e1] rounded-full py-2 px-2.5 shadow-[0_4px_12px_rgba(0,0,0,0.08)] flex items-center gap-1.5 text-xs font-semibold cursor-pointer transition-all duration-150 hover:bg-slate-50 hover:-translate-y-0.5 z-10"
        >
          ⬇
        </button>
      )}
      ...
      >
        <div className="flex items-center bg-white rounded-md px-3.5 gap-3 h-11 transition-colors duration-150 border border-solid border-[#cbd5e1] focus-within:border-black">
          <input
            type="text"
            value={inputValue}
          ...
          <button
            type="submit"
            title="전송"
            className="border-none bg-transparent text-[#18181b] cursor-pointer flex items-center justify-center h-8 w-8 rounded-full bg-[#f4f4f5] transition-colors duration-150 hover:bg-[#e4e4e7] shrink-0"
          >
            <svg
              width="18"
              height="18"
